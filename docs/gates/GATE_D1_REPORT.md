...
## Addendum (2026-07-26, post-report): publisher identity created (operator-ordered)

The operator explicitly ordered publisher-identity creation. Executed outside the D1 grant as its own authorized action: `createProfile(admin 0x534117…BbbFdE, [], "okf-mainnet", nodeId, 0)` signed by operational wallet `0x633E…e2Ab` through the node's own `EVMChainAdapter.ensureProfile` with `stakeAmount: 0` (no conviction stake — publish attribution needs none; the 50k-TRAC default was deliberately skipped). Tx `0x8fe764cf799baf15c8ac72c8839ae6050e167549553f2f5629e8a75af14970f0`, block 49142839, status success; receipt initially lost to a proxy timeout and recovered by read-back (no retry). Daemon re-read via `POST /api/identity/ensure`: **identityId 65, hasIdentity: true** (live on `/api/status`).

**D3 readiness update:** identity blocker CLEARED. Remaining before D3 GO: operator approval per the D3 block (spend ceilings etc.); live cost quote vs balances at preflight (113 TRAC ample for a small KA; 0.00335 ETH remaining after the profile tx — verify against the quote); `asyncPublisher` stays disabled by design — the D3 flow uses the sync `vm/publish` path, which does not require it.
